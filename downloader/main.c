...
static gpointer check_internet_thread(gpointer data) {
    ...
    int ret = system("wget -q --spider --timeout=5 --tries=1 https://sourceforge.net/projects/neko-void/files/Kasha-Installer/kasha-neko.tar/download 2>&1");
    ...
    if (wstat != 0) {
        ...
        app->status = 0;
        ret = system("cd /tmp && wget -q --timeout=30 https://sourceforge.net/projects/neko-void/files/Kasha-Installer/kasha-neko.tar/download -O installer.tar 2>&1");
