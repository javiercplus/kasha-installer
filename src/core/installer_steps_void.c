...
void void_reconfigure_base(AppData *app, const char *TARGETDIR) {
    ...
    run_sync(app, "chroot %s xbps-install -Syu --repository=https://repo-de.voidlinux.org/current/ --repository=https://sourceforge.net/projects/neko-void/files/repo kpm kyoz xmirror Neko-Wizard ", TARGETDIR);
}
...
void void_install_grub_efi_pkg(AppData *app, const char *TARGETDIR) {
    ...
    if (strcmp(app->efi_target, "x86_64-efi") == 0) {
        run_sync(app, "chroot %s xbps-install -Sy --repository=https://repo-de.voidlinux.org/current/ grub-x86_64-efi", TARGETDIR);
    } else {
        run_sync(app, "chroot %s xbps-install -Sy --repository=https://repo-de.voidlinux.org/current/ grub-i386-efi", TARGETDIR);
    }
}
...
void void_install_osprober(AppData *app, const char *TARGETDIR) {
    ...
    run_sync(app, "chroot %s xbps-install -Sy --repository=https://repo-de.voidlinux.org/current/ os-prober ntfs-3g 2>/dev/null || true", TARGETDIR);
}
...
void void_install_grub_bios_pkg(AppData *app, const char *TARGETDIR) {
    ...
    run_sync(app, "chroot %s xbps-install -Sy --repository=https://repo-de.voidlinux.org/current/ grub-i386-pc", TARGETDIR);
}
